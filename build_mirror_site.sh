...
wget -c -q --show-progress "$urlDebian13/SHA513SUMS" -O "$dirDebian13/SHA513SUMS"
...
wget -c -q --show-progress "$urlDebian11/SHA511SUMS" -O "$dirDebian11/SHA511SUMS"
...
LatestAlpineCloud_BiosCloudinit=$(curl -s "$urlAlpineCloud" | grep -oE 'generic_alpine-[0-9.]+-x86_64-bios-cloudinit-r[0-9]+\.qcow2' | sort -V | tail -n 1)
LatestAlpineCloud_UefiCloudinit=$(curl -s "$urlAlpineCloud" | grep -oE 'generic_alpine-[0-9.]+-x86_64-uefi-cloudinit-r[0-9]+\.qcow2' | sort -V | tail -n 1)
LatestAlpineCloud_BiosTiny=$(curl -s "$urlAlpineCloud" | grep -oE 'generic_alpine-[0-9.]+-x86_64-uefi-tiny-r[0-9]+\.qcow2' | sort -V | tail -n 1)
LatestAlpineCloud_UefiTiny=$(curl -s "$urlAlpineCloud" | grep -oE 'generic_alpine-[0-9.]+-x86_64-uefi-tiny-r[0-9]+\.qcow2' | sort -V | tail -n 1)
